...
const { isValidId, sanitizeId, isValidPhoneNumber } = require('../utils/validation');
// Security: CSRF is handled at the server level via same-origin validation (see index.js)
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        // A verified type controls the final extension, never the client filename.
...
function mediaTypeFromSignature(filePath, declaredMime) {
    // The path is generated server-side by multer (randomUUID in mediaDir),
    // but contain it anyway: never open a file that resolves outside mediaDir.
    ...
    const startsWith = (bytes) => bytesRead >= bytes.length && header.subarray(0, bytes.length).equals(Buffer.from(bytes));

    if (startsWith([0xff, 0xd8, 0xff])) return { extension: '.jpg', kind: 'image' };
    if (startsWith([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])) return { extension: '.png', kind: 'image' };
    if (header.subarray(0, 6).toString('ascii') === 'GIF87a' || header.subarray(0, 6).toString('ascii') === 'GIF89a') return { extension: '.gif', kind: 'image' };
    if (header.subarray(0, 4).toString('ascii') === 'RIFF' && header.subarray(8, 12).toString('ascii') === 'WEBP') return { extension: '.webp', kind: 'image' };
    if (header.subarray(0, 5).toString('ascii') === '%PDF-') return { extension: '.pdf', kind: 'document' };
    const isOle = startsWith([0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1]);
    if (isOle && ['application/msword', 'application/vnd.ms-excel'].includes(declaredMime)) {
        return { extension: MIME_TO_EXTENSION[declaredMime], kind: 'document' };
    }
    const isZip = startsWith([0x50, 0x4b, 0x03, 0x04]) || startsWith([0x50, 0x4b, 0x05, 0x06]) || startsWith([0x50, 0x4b, 0x07, 0x08]);
    if (isZip && ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'].includes(declaredMime)) {
        return { extension: MIME_TO_EXTENSION[declaredMime], kind: 'document' };
    }
    return null;
}

function initializeApi(sessions, sessionTokens, createSession, getSessionsDetails, deleteSession, log, userManager, activityLogger, broadcast = null) {
    ...
    const apiLimiter = rateLimit({
        ...
        // Do not trust proxy headers unless express 'trust proxy' is configured
    ...
    // Resolve the authenticated dashboard user, re-syncing role/status from the
    // users table so demotions and deactivations take effect immediately.
    // Legacy admin sessions (env password) are exempt from the DB check.
    ...
    router.post('/sessions', async (req, res) => {
        ...
        if (!currentUser) {
            ...
            if (!requiredMasterKey) {
                log('Session creation rejected: MASTER_API_KEY not configured', 'SYSTEM', {
                    event: 'auth-failed',
                ...
                return res.status(503).json({
                    status: 'error',
                    message: 'MASTER_API_KEY is not configured on the server'
                });
            }
        ...
        const existingSessionRecord = SessionModel.findById(sanitizedSessionId);

        // Check session limit for new sessions
        if (!existingSessionRecord) {
            const maxSessions = parseInt(process.env.MAX_SESSIONS || '10', 10);
            if (SessionModel.countActive() >= maxSessions) {
                return res.status(400).json({
                    status: 'error',
                    message: `Maximum active session limit reached (${maxSessions})`
                });
            }
        }

        // Ownership check for existing sessions
        if (existingSessionRecord && currentUser && currentUser.role !== 'admin') {
            if (existingSessionRecord.owner_email && existingSessionRecord.owner_email !== currentUser.email) {
                return res.status(403).json({
                    status: 'error',
                    message: 'Session ID belongs to another user'
                });
            }
        }
    ...
    router.get('/sessions', (req, res) => {
        ...
        // API access with a valid session bearer token: scope the listing to
        // the token's own session (least privilege; tokens never echoed back)
        const authHeader = req.headers['authorization'];
        const token = authHeader && authHeader.split(' ')[1];
        if (token && Array.from(sessionTokens.values()).includes(token)) {
            const Session = require('../models/Session');
            const owned = Session.findByToken(token);
            if (owned) {
                const live = sessions.get(owned.id);
                const enriched = {
                    ...owned,
                    sessionId: owned.id,
                    isConnected: live ? live.status === 'CONNECTED' : false
                };
                delete enriched.token;
    ...
    router.get('/sessions/:sessionId/qr', async (req, res) => {
        ...
        try {
            ...
            return res.status(200).json({ status: 'success', message: 'QR code generation started' });
        } catch (err) {
            log('API error', 'SYSTEM', { event: 'api-error', error: err.message, endpoint: req.originalUrl });
            return res.status(500).json({ status: 'error', message: `Failed to generate QR code: ${err.message}` });
        }
    });
    ...
    if (!encryptionKey || encryptionKey === 'your_generated_64_character_hex_key') {
        console.error('[API] FATAL: TOKEN_ENCRYPTION_KEY is not properly configured in .env');
        console.error('[API] Generate one with: node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'hex\'))"');
        throw new Error('TOKEN_ENCRYPTION_KEY must be set to a valid 64-character hex key');
    }
    ...
    const recipientListManager = new RecipientListManager(encryptionKey);

    // Broadcast campaign progress/status to authenticated WebSocket clients
    if (typeof broadcast === 'function') {
        campaignSender.on('progress', (data) => {
            const status = campaignSender.getCampaignStatus(data.campaignId);
            broadcast({
                type: 'campaign-progress',
                campaignId: data.campaignId,
                statistics: status ? status.statistics : null
            });
        });
        campaignSender.on('status', (data) => {
            broadcast({
...
+9988776655,Charlie Davis,Product Manager,Innovation Labs`;
    ...
    router.post('/campaigns', checkCampaignAccess, async (req, res) => {
        try {
            ...
            // SSRF guard: media URLs are fetched server-side by Baileys
            if (campaignData.message?.mediaUrl) {
                campaignData.message.mediaUrl = await validateExternalUrl(campaignData.message.mediaUrl);
            }
    ...
    router.put('/campaigns/:id', checkCampaignAccess, async (req, res) => {
        ...
        try {
            ...
            // SSRF guard: media URLs are fetched server-side by Baileys
            if (req.body.message?.mediaUrl) {
                req.body.message.mediaUrl = await validateExternalUrl(req.body.message.mediaUrl);
            }
    ...
    // Webhook setup endpoint (persisted in the database, SSRF-guarded)
    router.post('/webhook', async (req, res) => {
        ...
        try {
            ...
            setWebhookUrl(sessionId, validatedUrl);
            log('Webhook URL updated', sessionId, { event: 'webhook-updated', sessionId, url: validatedUrl });
            res.status(200).json({ status: 'success', message: `Webhook URL for session ${sessionId} updated` });
        } catch (err) {
            log('API error', 'SYSTEM', { event: 'api-error', error: err.message, endpoint: req.originalUrl });
    ...
    router.get('/webhook', (req, res) => {
        ...
        const url = getWebhookUrl(sessionId) || null;
    ...
    router.post('/messages', async (req, res) => {
        ...
        if (messages.length > 100) {
            return res.status(400).json({ status: 'error', message: 'Maximum 100 messages per request' });
        }
        const results = [];
        ...
        for (const msg of messages) {
            ...
            if (!isValidPhoneNumber(to)) {
                results.push({ status: 'error', message: 'Invalid recipient format. Use E.164 format (e.g., 6281234567890) or JID format (e.g., 6281234567890@s.whatsapp.net).' });
                continue;
            }
            ...
            if (type === 'image' && image) {
                ...
                if (image.link) {
                    try {
                        await validateExternalUrl(image.link);
                    } catch (err) {
                        results.push({ status: 'error', message: `Invalid image URL: ${err.message}` });
                        continue;
                    }
                }
            ...
            if (type === 'document' && document) {
                ...
                if (document.link) {
                    try {
                        await validateExternalUrl(document.link);
                    } catch (err) {
                        results.push({ status: 'error', message: `Invalid document URL: ${err.message}` });
                        continue;
                    }
                }
        ...
        if (activityLogger) {
            ...
            const logUserEmail = userEmail || (sessionOwner ? sessionOwner.email : 'api-user');
    ...
    // Group Scraper Endpoint: GET /api/v1/sessions/:id/groups
    router.get('/sessions/:id/groups', async (req, res) => {
        try {
            const whatsappService = require('../services/whatsapp');
            const sessionId = req.params.id;
            const groups = await whatsappService.getJoinedGroups(sessionId);
    ...
    router.post('/legacy/send-message', legacyLimiter, validateToken, express.json(), async (req, res) => {
        ...
        // Input validation using E.164 phone validation
        if (!isValidPhoneNumber(number)) {
            return res.status(400).json({ status: 'error', message: 'Invalid phone number format. Use 8-15 digits.' });
        }
...
module.exports = { initializeApi, getWebhookUrl, mediaTypeFromSignature }; 